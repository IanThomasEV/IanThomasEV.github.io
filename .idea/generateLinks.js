    ...
    fs.readdir(mainFolder, (err, files) => {
        ...
        files.forEach((file) => {
            ...
            fs.stat(filePath, (error, stat) => {
                ...
                if (stat.isDirectory()) {
                    const indexPath = path.join(filePath, 'index2.html');
                    if (fs.existsSync(indexPath)) {
                        fs.readFile(indexPath, 'utf8', (err, data) => {
                            if (err) {
                                console.error('Error reading index2.html file.', err);
                                return;
                            }
                            ...
                            if (pendingFiles === 0) {
                                ...
                                fs.writeFile(path.join(mainFolder, 'index2.html'), htmlContent, (err) => {
                                    if (err) throw err;
